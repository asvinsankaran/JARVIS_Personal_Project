...
void movemotors(){
  ...
  Serial.println("----^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^--------------------------------------");
  if(float((A*A+pos1*pos1-pos2*pos2)/(2*pos1*A))>=1){
    float gamma = acos(1.0);
  }
  else{
    float gamma = acos(float((A*A+pos1*pos1-pos2*pos2)/(2*pos1*A)));
  }
  ...
  float c = a[0]*P1[0]+a[1]*P1[1];
  float b = a[0]*P2[0]+a[1]*P2[1];
  c = -1*c;
  b = -1*b;
  ...
  float squareerror = sqrt(sq(destinationy-Y)+sq(destinationz-Z));
  ...
  squareerror = sqrt(sq(destinationy-Y)+sq(destinationz-Z));
  if(squareerror<30){
    analogWrite (M1A, 0);
    analogWrite (M1B, 0);
    analogWrite (M2A, 0);
